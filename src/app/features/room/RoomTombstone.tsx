  ...
  return (
    <RoomInputPlaceholder alignItems="Center" gap="600" className={css.RoomTombstone}>
      <Box direction="Column" grow="Yes">
        <Text size="T400">{body || t('Organisms.RoomTombstone.replaced')}</Text>
        {joinState.status === AsyncStatus.Error && (
          <Text style={{ color: color.Critical.Main }} size="T200">
            {(joinState.error as any)?.message ?? t('Organisms.RoomTombstone.join_failed')}
          </Text>
        )}
      </Box>
      <Box shrink="No">
        ...
        joinState.status === AsyncStatus.Success ? (
          <Button onClick={handleOpen} size="300" variant="Success" fill="Solid" radii="300">
            <Text size="B300">{t('Organisms.RoomTombstone.open_new')}</Text>
          </Button>
        ) : (
          <Button
            onClick={handleJoin}
          ...
          >
            <Text size="B300">{t('Organisms.RoomTombstone.join_new')}</Text>
          </Button>
        )}
      </Box>
